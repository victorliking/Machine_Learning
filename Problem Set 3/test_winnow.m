...
for m_index = 1:numel(m)
    ...
    for idx = 1:numel(prom_demot_parameters)
        [weights] = winnow(d_1_x,d_1_y,prom_demot_parameters(idx));
        accuracy_pc = calaccuracy(d_2_x,d_2_y,weights,-1*n);
        if(accuracy_pc > max_accuracy)
            max_accuracy = accuracy_pc;
    ...
    for idx = 1:numel(prom_demot_parameters)
        for idy = 1:numel(margins)
            [weights] = winnow_margin(d_1_x,d_1_y,prom_demot_parameters(idx),margins(idy));
            accuracy_pc = calaccuracy(d_2_x,d_2_y,weights,-1*n);
            if(accuracy_pc > max_accuracy)
                max_accuracy = accuracy_pc;
                prom_demot_val = prom_demot_parameters(idx);
...
for m_index = 1:numel(m)
    ...
    [weights] = winnow(train_x,train_y,winnow_prom_demot_par(1,m_index));
    accuracy_winnow(m_index) = calaccuracy(test_x,test_y,weights,-1*n);
        
    % For Winnow with Margin (With the best combination of promotion/demotion parameters and the margin:
    [weights] = winnow_margin(train_x,train_y,winnow_margin_prom_demot_par(1,m_index),winnow_margin_margin(1,m_index));
    accuracy_winnow_margin(m_index) = calaccuracy(test_x,test_y,weights,-1*n);
    %{    
    % For Adagrad. (With the best learning rate.):
    [weights,theta] = adagrad_train(train_x,train_y,adagrad_learning_rate(1,m_index));
    accuracy_adagrad(m_index) =
    accuracy_test(test_x,test_y,weights,theta);
    %}
end
